 ...
 */
const calculateEndAt = (originalStart: IsoDateTime, originalEnd: IsoDateTime, newStart: IsoDateTime): IsoDateTime => {
    const duration = new Date(originalEnd).getTime() - new Date(originalStart).getTime();
    return new Date(new Date(newStart).getTime() + duration).toISOString() as IsoDateTime;
};
    ...
    for (const master of masterCards) {
        ...
        try {
            ...
            for (const occurrenceDate of occurrences) {
                const occurrenceIso = occurrenceDate.toISOString() as IsoDateTime;
                const occurrenceTime = occurrenceDate.getTime();
                
                const override = exceptions.find(e => {
                    if (!e.parent_routine_id || e.parent_routine_id !== master.id || !e.original_recurrence_date) return false;
                    return new Date(e.original_recurrence_date).getTime() === occurrenceTime;
                });
                ...
                if (override) {
                    ...
                    instances.push({
                        ...
                        end_at: calculateEndAt(master.start_at, master.end_at, occurrenceIso),
                        _isVirtual: true,
                    });
                }
            }
