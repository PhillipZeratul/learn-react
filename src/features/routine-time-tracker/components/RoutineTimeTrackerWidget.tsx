...
export default function RoutineTimeTrackerWidget() {
  ...
  const endPress = async () => {
    ...
    if (dragState) {
      ...
      const finalEndMin =
        Math.round(
          (finalTop + finalHeight - TOP_MARGIN) / PIXELS_PER_MINUTE / 5
        ) * 5
      ...
      if (dragState.type === "routine") {
        ...
        const isRecurring =
          routine._isVirtual || !!routine.rrule || !!routine.parent_routine_id
  ...
  const handleMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (dragState) {
      ...
      if (dragState.mode === "top") {
        const requestedTop =
          dragState.initialStartMin * PIXELS_PER_MINUTE + TOP_MARGIN + deltaY
        const minTop = TOP_MARGIN
        const maxTop =
          (dragState.initialEndMin - 5) * PIXELS_PER_MINUTE + TOP_MARGIN
        newTop = Math.max(minTop, Math.min(maxTop, requestedTop))
        newHeight =
          dragState.initialEndMin * PIXELS_PER_MINUTE + TOP_MARGIN - newTop
