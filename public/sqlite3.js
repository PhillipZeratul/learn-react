    ...
    const genMsgId =
        ...
        function (msg) {
            return (
                msg.type +
                "#" +
                (idTypeMap[msg.type] = (idTypeMap[msg.type] || 0) + 1)
            )
        }
    const toss = (...args) => {
...
async function sqlite3InitModule(moduleArg = {}) {
    ...
    async function createWasm() {
        ...
        return receiveInstantiationResult(
    ...
    var MEMFS = {
        ...
        expandFileStorage(node, newCapacity) {
            ...
            newCapacity = Math.max(
                newCapacity,
                (prevCapacity * (prevCapacity < 1024 * 1024 ? 2 : 1.125)) >>> 0
            )
    ...
    var FS = {
        ...
        syncfs(populate, callback) {
            ...
            for (var mount of mounts)
                if (mount.type.syncfs) mount.type.syncfs(mount, populate, done)
                else done(null)
        },
        ...
        rename(old_path, new_path) {
            ...
            var lookup = FS.lookupPath(old_path, { parent: true }),
                old_dir = lookup.node,
                new_dir
        ...
        chmod(path, mode, dontFollow) {
            var node
            if (typeof path == "string")
                node = FS.lookupPath(path, { follow: !dontFollow }).node
            else node = path
    ...
    var ydayFromDate = (date) => {
        return (
            (isLeapYear(date.getFullYear())
                ...
                : MONTH_DAYS_REGULAR_CUMULATIVE)[date.getMonth()] +
            date.getDate() -
            1
        )
    }
    ...
    function assignWasmExports(wasmExports) {
        ...
        Module["_sqlite3__wasm_kvvfsMakeKey"] =
            wasmExports["sqlite3__wasm_kvvfsMakeKey"]
        ...
        Module["_sqlite3__wasm_kvvfs_decode"] =
            wasmExports["sqlite3__wasm_kvvfs_decode"]
        Module["_sqlite3__wasm_kvvfs_encode"] =
            wasmExports["sqlite3__wasm_kvvfs_encode"]
	...
	*/
    Module.runSQLite3PostLoadInit = async function (
        ...
        globalThis.sqlite3ApiBootstrap = async function sqlite3ApiBootstrap(
            ...
            ;["exports", "memory", "functionTable", "wasmfsOpfsDir"].forEach(
                (k) => {
			...
			*/
            const bigIntFits32 = (b) =>
                b >= -2147483647n - 1n && b <= 2147483647n
            /**
			Returns true if the given BigInt value is small enough to fit
			...
			*/
            Object.assign(wasm, {
                ...
                pointerSize:
                    "number" === typeof config.exports.sqlite3_libversion()
                        ? 4
                        : 8,
			...
			*/
            ...
            }
            {
                ...
                for (const key of [keyAlloc, keyDealloc, keyRealloc])
                    if (!(wasm.exports[key] instanceof Function))
            ...
            wasm.pstack = Object.assign(Object.create(null), {
                ...
                allocChunks: function (n, sz) {
                    ...
                    let i = 1,
                        offset = sz
                    for (; i < n; ++i, offset += sz)
                        rc.push(wasm.ptr.add(mem, offset))
                    return rc
                },
                ...
                try {
					...
					*/
                    ...
                    rc = nOut
                        ? wasm
                              .heap8u()
                              .slice(Number(pOut), Number(pOut) + Number(nOut))
                        : new Uint8Array()
                    return rc
                } finally {
			...
			*/
            capi.sqlite3_js_db_vfs = (dbPointer, dbName = wasm.ptr.null) =>
			...
			*/
            capi.sqlite3_js_posix_create_file = function (
                filename,
                data,
                dataLen
            ) {
				...
				*/
                    tgt.push(
                        capi.sqlite3_value_to_js(
                            wasm.peekPtr(
                                wasm.ptr.add(pArgv, wasm.ptr.size * i)
                            ),
		...
		*/
        ...
        globalThis.sqlite3ApiBootstrap.initializers.push(function (sqlite3) {
            sqlite3.version = {
                ...
                sourceId:
                    "2026-01-30 06:37:34 407724c4e80efdf93d885e95b5209a100a3f470fe0298138be57201f65f9817e",
        ...
        globalThis.WhWasmUtilInstaller = function WhWasmUtilInstaller(target) {
			...
			*/
            target.peekPtr = (...ptr) =>
                target.peek(1 === ptr.length ? ptr[0] : ptr, __ptrIR)
            /**
			...
			*/
            const typedArrayToString = (typedArray, begin, end) =>
                cache.utf8Decoder.decode(typedArrayPart(typedArray, begin, end))
			...
			*/
            target.cstrncpy = function (tgtPtr, srcPtr, n) {
                ...
                let i = 0,
                    ch
                const tgtNumber = Number(tgtPtr),
                    srcNumber = Number(srcPtr)
			...
			*/
            target.scopedAllocPop = function (state) {
                ...
                for (let p; (p = state.pop()); )
                    if (target.functionEntry(p)) target.uninstallFunction(p)
            ...
            target.cArgvToJs = (argc, pArgv) => {
                ...
                for (let i = 0; i < argc; ++i) {
                    const arg = target.peekPtr(
                        __ptrAdd(pArgv, target.ptr.size * i)
                    )
			...
			*/
            ...
            for (const t of [
                ...
                xResult.set(
                    ...
                    xArg.get(t) ||
                        toss(
                            "Maintenance required: missing arg converter for",
                            t
                        )
                )
            }
			...
			*/
            xArg.FuncPtrAdapter = class FuncPtrAdapter extends (
                ...
                constructor(opt) {
                    ...
                    this.singleton =
                        "singleton" === this.bindScope ? [] : void 0
                    this.callProxy =
				...
				*/
                contextMap(key) {
                    const cm =
                        this.__cmap || (this.__cmap = /* @__PURE__ */ new Map())
                    let rc = cm.get(key)
			...
			*/
            cache.xWrap.convertResult = (t, v) =>
                null === t ? v : t ? __xResultAdapterCheck(t)(v) : void 0
            /**
			...
			*/
            cache.xWrap.convertResultNoCheck = (t, v) =>
                null === t ? v : t ? xResult.get(t)(v) : void 0
            /**
		...
		*/
        ...
        globalThis.Jaccwabyt = function StructBinderFactory(config) {
            ...
            }
            {
                ...
                if (h instanceof WebAssembly.Memory)
                    ...
                    toss(
                        "config.heap must be WebAssembly.Memory instance or",
            ...
            config.realloc
            const log = config.log || console.debug.bind(console),
			...
			*/
            ...
            const __propThrowOnSet = function (structName, propName) {
                return () =>
                    toss(sPropName(structName, propName), "is read-only.")
            }
            /**
			...
			*/
            ...
            const __freeStruct = function (ctor, obj, m) {
                ...
                if (!m && !(m = ii.p)) {
                    console.warn(
                        "Cannot(?) happen: __freeStruct() found no instanceInfo"
                    )
                    return
                }
            ...
            const __affirmNotASig = function (ctx, key) {
                looksLikeASig(key) &&
                    toss(
                        ...
                        "(",
                        key,
                        ") collides with a data type signature."
                    )
            }
			...
			*/
            const __adaptStruct2 = function (who, key) {
                const si =
                    "string" === typeof key ? __adaptor(who, 2, key) : key
                if ("object" !== typeof si)
                    toss(
                        "Invalid struct mapping object. Arg =",
                        key,
                        JSON.stringify(si)
                    )
                return si
            ...
            const __lookupMember = function (
                ...
                if (!m && (memberPrefix || memberSuffix)) {
                    ...
                    if (!m && tossIfNotFound)
                        toss(
                            sPropName(
                                structInfo.name || structInfo.structName,
                                memberName
                            ),
			...
			*/
            StructType.prototype = Object.create(null, {
                ...
                memoryDump: rop(function () {
                    ...
                    return p
                        ? new Uint8Array(
                              heap().slice(
                                  Number(p),
                                  Number(p) + this.structInfo.sizeof
                              )
                          )
                        : null
                }),
                ...
                zeroOnDispose: {
                    ...
                    get: function () {
                        return (
                            getInstanceHandle(this, false)?.zod ??
                ...
                pointer: {
                    ...
                    set: () =>
                        toss(
                            "Cannot assign the 'pointer' property of a struct."
                        ),
                },
			...
			*/
            const memberGetterProxy = function (si) {
                return (
                    si.get ||
                    (si.adaptGet ? StructBinder.adaptGet(si.adaptGet) : void 0)
                )
            }
            /**
			...
			*/
            const memberSetterProxy = function (si) {
                return (
                    si.set ||
                    (si.adaptSet ? StructBinder.adaptSet(si.adaptSet) : void 0)
                )
            }
            /**
			...
			*/
            const makeMemberStructWrapper = function callee(ctor, name, si) {
				...
				*/
                const __innerStructs = (callee.innerStructs ??=
                    /* @__PURE__ */ new Map())
                const key = ctor.memberKey(name)
                if (void 0 !== si.signature)
                    toss(
                        "'signature' cannot be used on an embedded struct (",
                        ctor.structName,
                        ".",
                        key,
                        ")."
                ...
                if (memberSetterProxy(si))
                    toss(
                        "'set' and 'adaptSet' are not permitted for nested struct members."
                    )
                ...
                if (si.readOnly)
                    ...
                    prop.set = function (v) {
						...
						*/
                        ...
                        if (!this.pointer)
                            toss(
                                "Cannot set native property on a disposed",
                                this.structName,
                                "instance."
                            )
                        ...
                        if (null === v || void 0 === v) v = __NullPtr
                            ...
                            if (
                                ...
                                toss(
                                    "Invalid value for pointer-type",
                                    xPropName + "."
                                )
                        new DataView(
                            heap().buffer,
			...
			*/
            const StructBinderImpl = function StructBinderImpl(
				...
				*/
                ...
                if (!structName)
                    toss("One of 'name' or 'structName' are required.")
                if (si.adapt) {
                ...
                const debugFlags = rop(
                    SBF.__makeDebugFlags(StructBinder.debugFlags)
                )
                ...
                if (!autoCalc) {
                    if (!si.sizeof)
                        toss(
                            structName,
                            "description is missing its sizeof property."
                        )
                    si.offset ??= 0
                } else si.offset ??= 0
                ...
                if (si.sizeof === 1)
                    ...
                    if (0 !== si.sizeof % 4) {
                        ...
                        toss(
                            structName,
                            "sizeof is not aligned. sizeof=" + si.sizeof
                        )
                    }
                    if (0 !== si.offset % 4) {
                        console.warn("Invalid struct member description", si)
                        toss(
                            structName,
                            "offset is not aligned. offset=" + si.offset
                        )
                    }
                }
                if (si.sizeof < offset) {
                    console.warn(
                        "Suspect struct description:",
                        si,
                        "offset =",
                        offset
                    )
                    toss(
                        "Mismatch in the calculated vs. the provided sizeof/offset info.",
                        "Expected sizeof",
                        offset,
                        "but got",
                        si.sizeof,
        ...
        globalThis.sqlite3ApiBootstrap.initializers.push(function (sqlite3) {
            ...
            delete globalThis.Jaccwabyt
            {
				...
				*/
                ...
                wasm.xWrap.argAdapter("sqlite3_filename", __xArgPtr)(
                    ...
                    __xArgPtr(
                        v instanceof (sqlite3?.oo1?.Stmt || nilType)
                            ? v.pointer
                            : v
                    )
                )("sqlite3*", (v) =>
                    __xArgPtr(
                        v instanceof (sqlite3?.oo1?.DB || nilType)
                            ? v.pointer
                            : v
                    )
                )("sqlite3_vfs*", (v) => {
                    ...
                    return __xArgPtr(
                        v instanceof (capi.sqlite3_vfs || nilType)
                            ? v.pointer
                            : v
                    )
                })
				...
				*/
                util.sqlite3__wasm_db_error = function (
                    ...
                    return (
                        capi.sqlite3_set_errmsg(pDb, resultCode, message) ||
                        resultCode
                    )
                }
            }
			...
			*/
            ...
            const __argStr = (str) =>
                wasm.isPtr(str) ? wasm.cstrToJs(str) : str
                    ...
                    switch (typeof xSql) {
                        ...
                        default:
                            return util.sqlite3__wasm_db_error(
                                pDb,
                                capi.SQLITE_MISUSE,
                                "Invalid SQL argument type for sqlite3_prepare_v2/v3(). typeof=" +
                                    typeof xSql
                            )
                    }
                }
        ...
        globalThis.sqlite3ApiBootstrap.initializers.push(function (sqlite3) {
            ...
            const __dbTraceToConsole = wasm.installFunction(
                ...
                function (t, c, p, x) {
                    if (capi.SQLITE_TRACE_STMT === t)
                        console.log(
                            "SQL TRACE #" + ++this.counter,
                            "via sqlite3@" +
                                c +
                                "[" +
                                capi.sqlite3_db_filename(c, null) +
                                "]",
                            wasm.cstrToJs(x)
                        )
                }.bind({ counter: 0 })
            ...
            const dbCtorHelper = function ctor(...args) {
                ...
                if ((pDb = opt["sqlite3*"])) {
                    ...
                    let fn = opt.filename,
                        ...
                        flagsStr = opt.flags
                    if (
                        ("string" !== typeof fn && !wasm.isPtr(fn)) ||
                ...
                if (!opt["sqlite3*"])
                    try {
                        const postInitSql =
                            __vfsPostOpenCallback[
                                capi.sqlite3_js_db_vfs(pDb) ||
                                    toss3(
                                        "Internal error: cannot get VFS for new db handle."
                                    )
                            ]
                        if (postInitSql)
                            /**
			...
			*/
            ...
            const isSupportedBindType = function (v) {
                let t =
                    BindTypes[null === v || void 0 === v ? "null" : typeof v]
                switch (t) {
			...
			*/
            const bindOne = function f(stmt, ndx, bindType, val) {
                ...
                let rc = 0
                switch (
                    null === val || void 0 === val ? BindTypes.null : bindType
                ) {
                    case BindTypes.null:
            ...
            Stmt.prototype = {
                ...
                get: function (ndx, asType) {
                    ...
                    switch (
                        ...
                        case capi.SQLITE_BLOB: {
                            const n = capi.sqlite3_column_bytes(
                                    this.pointer,
                                    ndx
                                ),
                                ptr = capi.sqlite3_column_blob(
                                    this.pointer,
                                    ndx
                                ),
                                rc = new Uint8Array(n)
                ...
                isReadOnly: function () {
                    return (
                        0 !== capi.sqlite3_stmt_readonly(affirmStmtOpen(this))
                    )
                },
            }
            {
		...
		*/
        ...
        globalThis.sqlite3ApiBootstrap.initializers.push(function (sqlite3) {
			...
			*/
            ...
            vtab.xError.errorReporter = sqlite3.config.error.bind(
                sqlite3.config
            )
            /**
                ...
                try {
                    ...
                    if (opt.catchExceptions) {
                        const fwrap = function (methodName, func) {
                            if (
                                ["xConnect", "xCreate"].indexOf(methodName) >= 0
                            )
                                return function (
                                    pDb,
                                    pAux,
		...
		*/
        globalThis.sqlite3ApiBootstrap.initializers.push(function (sqlite3) {
            ...
            util.assert(
                32 <= kvvfsMethods.$nKeySize,
                "unexpected kvvfsMethods.$nKeySize: " + kvvfsMethods.$nKeySize
            )
            /**
			...
			*/
            cache.memBuffer = (id = 0) =>
                (cache.buffer.pool[id] ??= wasm.alloc(cache.buffer.n))
			...
			*/
            const kvvfsKeyPrefix = (v) =>
                kvvfsIsPersistentName(v) ? "kvvfs-" + v + "-" : ""
            /**
			...
			*/
            const validateStorageName = function (n, mayBeJournal = false) {
                ...
                if (!len)
                    toss3(capi.SQLITE_MISUSE, "Empty name is not permitted.")
                ...
                if (len > maxLen)
                    toss3(
                        capi.SQLITE_RANGE,
                        "Storage name is too long. Limit =",
                        maxLen
                    )
                let i
                for (i = 0; i < len; ++i) {
			...
			*/
            const newStorageObj = (name, storage = void 0) =>
                Object.assign(Object.create(null), {
                    jzClass: name,
                    refc: 1,
                    deleteAtRefc0: false,
			...
			*/
            const installStorageAndJournal = (store) =>
                (cache.storagePool[store.jzClass] = cache.storagePool[
                    store.jzClass + "-journal"
                ] = store)
            /**
                        ...
                        if (f) {
                            ...
                            if (
                                ...
                                if (rc > 0) {
                                    ...
                                    wcache[args[0]] = ev.data[1] = heap.slice(
                                        Number(z1),
                                        wasm.ptr.addn(z1, rc)
                                    )
                                } else continue
			...
			*/
            const zKeyForStorage = (store, zClass, zKey) => {
                return zClass && store.keyPrefix
                    ? kvvfsMakeKey(zClass, zKey)
                    : zKey
            }
            const jsKeyForStorage = (store, zClass, zKey) =>
                wasm.cstrToJs(zKeyForStorage(store, zClass, zKey))
            const storageGetDbSize = (store) =>
                +store.storage.getItem(store.keyPrefix + "sz")
            /**
			...
			*/
                ...
                recordHandler: {
                    xRcrdRead: (zClass, zKey, zBuf, nBuf) => {
                        try {
                            ...
                            kvvfs?.log?.xRcrdRead &&
                                warn("xRcrdRead", jzClass, jXKey, nBuf, store)
                            ...
                            let i
                            for (i = 0; i < nV; ++i)
                                heap[wasm.ptr.add(zV, i)] =
                                    jV.codePointAt(i) & 255
                            heap.copyWithin(
                                Number(zBuf),
                                Number(zV),
                                wasm.ptr.addn(zV, i)
                    ...
                    xRcrdWrite: (zClass, zKey, zData) => {
                        try {
                            ...
                            kvvfs?.log?.xRcrdWrite &&
                                warn("xRcrdWrite", jxKey, store)
                    ...
                    xRcrdDelete: (zClass, zKey) => {
                        try {
                            const store = storageForZClass(zClass)
                            const jxKey = jsKeyForStorage(store, zClass, zKey)
                            kvvfs?.log?.xRcrdDelete &&
                                warn("xRcrdDelete", jxKey, store)
                ...
                vfs: {
                    xOpen: function (
                        ...
                        try {
                            ...
                            kvvfs?.log?.xOpen &&
                                debug("xOpen", jzClass, "flags =", flags)
                            ...
                            const rc = originalMethods.vfs.xOpen(
                                pProtoVfs,
                                zName,
                                pProtoFile,
                                flags,
                            ...
                            if (rc) return rc
                            let deleteAt0 = !!(
                                capi.SQLITE_OPEN_DELETEONCLOSE & flags
                            )
                            ...
                            if (s) {
                                ...
                                util.assert(
                                    !f.$isJournal,
                                    "Opening a journal before its db? " +
                                        jzClass
                                )
                                const nm = jzClass.replace(
                                    cache.rxJournalSuffix,
                                    ""
                                )
                                s = newStorageObj(nm)
                                ...
                                kvvfs?.log?.xOpen &&
                                    debug(
                                        "xOpen installed storage handle [",
                                        nm,
                                        nm + "-journal",
                                        "]",
                                        s
                    ...
                    xAccess: function (pProtoVfs, zPath, flags, pResOut) {
                        ...
                        try {
                            ...
                            if (s) {
                                const key =
                                    s.keyPrefix +
                                    (cache.rxJournalSuffix.test(jzPath)
                                        ? "jrnl"
                                        : "1")
                    ...
                    xGetLastError: function (pVfs, nOut, pOut) {
                        ...
                        if (e) {
                            ...
                            try {
                                ...
                                if (n > nOut)
                                    wasm.poke8(wasm.ptr.add(pOut, nOut, -1), 0)
                ...
                ioDb: {
                    ...
                    xFileControl: function (pFile, opId, pArg) {
                        ...
                        try {
                            ...
                            kvvfs?.log?.xFileControl &&
                                debug("xFileControl", h, "op =", opId)
                            if (
                                opId === capi.SQLITE_FCNTL_PRAGMA &&
                                kvvfsInternal.disablePageSizeChange
                            ) {
                                const zName = wasm.peekPtr(
                                    wasm.ptr.add(pArg, wasm.ptr.size)
                                )
                                if ("page_size" === wasm.cstrToJs(zName)) {
                                    ...
                                    const zVal = wasm.peekPtr(
                                        wasm.ptr.add(pArg, 2 * wasm.ptr.size)
                                    )
                                    if (zVal) {
                                        kvvfs?.log?.xFileControl &&
                                            warn(
                    ...
                    xSync: function (pFile, flags) {
                        ...
                        try {
                            ...
                            if (0 == rc && h.store.listeners)
                                notifyListeners("sync", h.store, true)
                            return rc
                        } catch (e) {
                            error("xSync", e)
                            return cache.setError(e)
                    ...
                    xRead: function (pFile, pTgt, n, iOff64) {
                        ...
                        try {
                            ...
                            return originalMethods.ioDb.xRead(
                                pFile,
                                pTgt,
                                n,
                                iOff64
                    ...
                    xWrite: function (pFile, pSrc, n, iOff64) {
                        ...
                        try {
                            ...
                            return originalMethods.ioDb.xWrite(
                                pFile,
                                pSrc,
                                n,
                                iOff64
            ...
            try {
                util.assert(
                    cache.buffer.n > 1024 * 129,
                    "Heap buffer is not large enough"
                )
                ...
                for (const e of Object.entries(methodOverrides.ioDb)) {
                    ...
                    originalMethods.ioDb[k] =
                        wasm.functionEntry(pIoDb[km]) ||
                        util.toss("Missing native pIoDb[", km, "]")
                    pIoDb[km] = wasm.installFunction(
                        pIoDb.memberSignature(k),
                        f
                    )
                }
                for (const e of Object.entries(methodOverrides.ioJrnl)) {
                    ...
                    originalMethods.ioJrnl[k] =
                        wasm.functionEntry(pIoJrnl[km]) ||
                        util.toss("Missing native pIoJrnl[", km, "]")
                    if (true === f)
                        pIoJrnl[km] =
                            pIoDb[km] ||
                            util.toss("Missing copied pIoDb[", km, "]")
                    else
                        pIoJrnl[km] = wasm.installFunction(
                            pIoJrnl.memberSignature(k),
                            f
                        )
                }
            } finally {
			...
			*/
            const sqlite3_js_kvvfs_export = function callee(...args) {
                ...
                if (1 === args.length && "object" === typeof args[0])
                    ...
                    opt = Object.assign(Object.create(null), { name: args[0] })
                const store = opt ? storageForZClass(opt.name) : null
                if (!store)
                    toss3(
                        capi.SQLITE_NOTFOUND,
                        "There is no kvvfs storage named",
                        opt?.name
                    )
                const s = store.storage
                ...
                for (; i < n; ++i) {
                    ...
                    if (!keyPrefix || k.startsWith(keyPrefix)) {
                        ...
                        switch (kk) {
                            case "jrnl":
                                if (opt.includeJournal)
                                    rc.journal = s.getItem(k)
                                break
                            case "sz":
                                rc.size = +s.getItem(k)
                                break
                            default:
                                ...
                                if (opt.decodePages) {
                                    ...
                                    let i = 0
                                    for (; i < n; ++i)
                                        heap[wasm.ptr.add(z, i)] =
                                            spg.codePointAt(i) & 255
                ...
                Object.keys(pages)
                    .map((v) => +v)
                    .sort()
                    .forEach((v) => rc.pages.push(pages[v]))
                return rc
            }
            /**
                ...
                try {
                    ...
                    if (exp.pages[0] instanceof Uint8Array)
                        exp.pages.forEach((u, ndx) => {
                            ...
                            util.assert(
                                0 === wasm.peek8(wasm.ptr.add(zEnc, rc)),
                        ...
                        exp.pages.forEach((v, ndx) =>
                            s.setItem(keyPrefix + (ndx + 1), v)
                        )
			...
			*/
            const sqlite3_js_kvvfs_unlink = function (name) {
                ...
                if (
                    ...
                    cache.rxJournalSuffix.test(name)
                )
                    return false
                if (
                    store.refc > store.files.length ||
                    0 === store.files.length
                ) {
			...
			*/
            ...
            if (globalThis.Storage) {
				...
				versions lack.
				*/
                capi.sqlite3_js_kvvfs_size = (which = "") =>
                    sqlite3_js_kvvfs_size(which)
                capi.sqlite3_js_kvvfs_clear = (which = "") =>
                    sqlite3_js_kvvfs_clear(which)
            }
            if (sqlite3.oo1?.DB) {
                /**
				...
				*/
                ...
                sqlite3.oo1.JsStorageDb.defaultStorageName = cache.storagePool
                    .session
                    ? "session"
                    : nameOfThisThreadStorage
				...
				*/
                jdb.prototype.clearStorage = function () {
                    return jdb.clearStorage(
                        this.affirmOpen().dbFilename(),
                        true
                    )
                }
            ...
            if (sqlite3.__isUnderTest && sqlite3.vtab) {
				...
				*/
                ...
                Object.assign(Object.create(ProtoCursor), {
                    rowid: 0,
                    names: Object.keys(cache.storagePool).filter(
                        (v) => !cache.rxJournalSuffix.test(v)
                    ),
                })
                const cursorState = function (cursor, reset) {
                    const o =
                        cursor instanceof capi.sqlite3_vtab_cursor
                            ? cursor
                            : VT.xCursor.get(cursor)
                    if (reset || !o.vTabState)
                        o.vTabState = Object.assign(
                            Object.create(ProtoCursor),
                            {
                                rowid: 0,
                                names: Object.keys(cache.storagePool).filter(
                                    (v) => !cache.rxJournalSuffix.test(v)
                ...
                const theModule = function f() {
                    return (f.mod ??=
                        new sqlite3.capi.sqlite3_module().setupModule({
                            ...
                            methods: {
                                ...
                                xColumn: function (pCursor, pCtx, iCol) {
                                    ...
                                    switch (iCol) {
                                        ...
                                        case cols.dbSize.colId:
                                            capi.sqlite3_result_int(
                                                pCtx,
                                                storageGetDbSize(store)
                                            )
                                            break
                                        default:
                                            capi.sqlite3_result_error(
                                                pCtx,
                                                "Invalid column id: " + iCol
                                            )
                                ...
                                xBestIndex: function (pVtab, pIdxInfo) {
                                    ...
                                    const pii = new capi.sqlite3_index_info(
                                        pIdxInfo
                                    )
                ...
                sqlite3.kvvfs.create_module = function (
                    pDb,
                    name = "sqlite_kvvfs"
                ) {
                    return capi.sqlite3_create_module(
                        pDb,
                        name,
                        theModule(),
                        wasm.ptr.null
			...
			*/
            const installOpfsVfs = function callee(options) {
                if (!globalThis.SharedArrayBuffer || !globalThis.Atomics)
                    ...
                    return Promise.reject(
                        /* @__PURE__ */ new Error("Missing required OPFS APIs.")
                    )
                ...
                return new Promise(function (promiseResolve_, promiseReject_) {
					...
					*/
                    const opRun = (op, ...args) => {
                        const opNdx =
                            state.opIds[op] || toss("Invalid op ID:", op)
					...
					*/
                    const ioSyncWrappers = {
                        ...
                        xUnlock: function (pFile, lockType) {
                            ...
                            let rc = 0
                            if (
                                capi.SQLITE_LOCK_NONE === lockType &&
                                f.lockType
                            )
					...
					*/
                        ...
                        xOpen: function f(
                            ...
                            if (0 === zName) zName = randomFilename()
                                ...
                                if (
                                    ...
                                    opfsFlags |=
                                        state.opfsFlags.OPFS_UNLOCK_ASAP
                                if (
                                    capi.sqlite3_uri_boolean(
                                        zName,
                                        "delete-before-open",
                    ...
                    if (!opfsVfs.$xRandomness)
                        vfsSyncWrappers.xRandomness = function (
                            pVfs,
                            nOut,
                            pOut
                        ) {
					...
					*/
                    opfsUtil.getResolvedPath = function (filename, splitIt) {
                        const p = new URL(filename, "file://irrelevant")
                            .pathname
					...
					*/
                    opfsUtil.entryExists = async function (fsEntryName) {
                        try {
                            const [dh, fn] =
                                await opfsUtil.getDirForFilename(fsEntryName)
                    ...
                    opfsUtil.traverse = async function (opt) {
                        ...
                        ;(async function callee(dirHandle, depth) {
                            for await (const handle of dirHandle.values())
                                if (
                                    false ===
                                    opt.callback(handle, dirHandle, depth)
                                )
                                    return false
                                else if (
                                    opt.recursive &&
					...
					*/
                    const importDbChunked = async function (
                        ...
                        const [hDir, fnamePart] =
                            await opfsUtil.getDirForFilename(filename, true)
                        let sah = await (
                                await hDir.getFileHandle(fnamePart, {
                                    create: true,
                                })
                            ).createSyncAccessHandle(),
                            nWrote = 0,
					...
					*/
                    ...
                    if (sqlite3.oo1) {
                        const OpfsDb = function (...args) {
                            const opt =
                                sqlite3.oo1.DB.dbCtorHelper.normalizeArgs(
                                    ...args
                                )
                            opt.vfs = opfsVfs.$zName
                            sqlite3.oo1.DB.dbCtorHelper.call(this, opt)
                        }
                        OpfsDb.prototype = Object.create(
                    ...
                    const sanityCheck = function () {
                        ...
                        try {
                            ...
                            const dbFile =
                                "/sanity/check/file" + randomFilename(8)
                        ...
                        switch (data.type) {
                            ...
                            case "opfs-async-inited":
                                ...
                                try {
                                    ...
                                    state.sabOPView = new Int32Array(
                                        state.sabOP
                                    )
        ...
        globalThis.sqlite3ApiBootstrap.initializers.push(function (sqlite3) {
			...
			*/
            const ioMethods = {
                ...
                xFileSize: function (pFile, pSz64) {
                    ...
                    pool.log(`xFileSize`)
                    const size =
                        pool.getOFileForS3File(pFile).sah.getSize() -
                        HEADER_OFFSET_DATA
                ...
                xRead: function (pFile, pDest, n, offset64) {
                    ...
                    try {
                        const nRead = file.sah.read(
                            wasm
                                .heap8u()
                                .subarray(Number(pDest), Number(pDest) + n),
                    ...
                    try {
                        return n ===
                            file.sah.write(
                                wasm
                                    .heap8u()
                                    .subarray(Number(pSrc), Number(pSrc) + n),
                                { at: HEADER_OFFSET_DATA + Number(offset64) }
                            )
                            ? 0
                            : toss("Unknown write() failure.")
                    } catch (e) {
                        return pool.storeErr(e, capi.SQLITE_IOERR)
                    }
                },
            ...
            const vfsMethods = {
                ...
                xCurrentTime: function (pVfs, pOut) {
                    wasm.poke(
                        pOut,
                        2440587.5 +
                            /* @__PURE__ */ new Date().getTime() / 864e5,
                        "double"
                    )
                    return 0
                },
			...
			*/
            class OpfsSAHPool {
                ...
                constructor(options = Object.create(null)) {
                    this.#verbosity =
                        options.verbosity ?? optionDefaults.verbosity
				...
				*/
                async addCapacity(n) {
                    for (let i = 0; i < n; ++i) {
                        ...
                        const ah = await (
                            await this.#dhOpaque.getFileHandle(name, {
                                create: true,
                            })
                        ).createSyncAccessHandle()
			...
			*/
            const apiVersionCheck = async () => {
                ...
                const close = (
                    await (
                        await dh.getFileHandle(fn, { create: true })
                    ).createSyncAccessHandle()
                ).close()
                await close
...
sqlite3InitModule = (function () {
	...
	*/
    ...
    if (!originalInit)
        throw new Error(
            "Expecting sqlite3InitModule to be defined by the Emscripten build."
        )
    /**
	...
	*/
    ...
    sIMS.debugModule = sIMS.urlParams.has("sqlite3.debugModule")
...
//#endregion
export {
    sqlite3_bundler_friendly_default as default,
    sqlite3_worker1_promiser_default as sqlite3Worker1Promiser,
}
